(function ($) {
    var pxl_widget_googlemap_handler = function ($scope, $) {
        $scope.find(".pxl-map-render").each(function () {
                ...
                if (Array.isArray(locations.markerlist)) {
                    for (var i = 0; i < locations.markerlist.length; i++) {
                        ...
                        if(locations.markerlist[i].title != '' || locations.markerlist[i].desc != ''){
                            item.markerdesc = '<div class="pxl-info-content"><h5>';
            ...
            function markerRender(map, locations) {
                ...
                if (location.length == 2) {
                    ...
                    google.maps.event.addListenerOnce(map, 'idle', function () {
                        setTimeout(function () {
                            $('img[usemap="#gmimap0"]').wrap("<div class='pxl-marker-icon'></div>");
                        }, 200);
                    });
    ...
    $(window).on('elementor/frontend/init', function () {
        elementorFrontend.hooks.addAction('frontend/element_ready/pxl_google_map.default', pxl_widget_googlemap_handler);
    });
})(jQuery);