...
class dentia_CSS_Generator {
	...
	function __construct() {
		$this->opt_name = dentia()->get_option_name();  
        ...
        add_action( 'wp_enqueue_scripts', array( $this, 'dentia_enqueue' ), 20 );
	}
	...
	function init() {
		...
		add_action( 'wp', array( $this, 'dentia_generate_with_dev_mode' ) );
		add_action( "redux/options/{$this->opt_name}/saved", function () {
			$this->dentia_generate_file_options();
		} );
	}

	function dentia_generate_with_dev_mode() {
		if ( $this->dev_mode === true ) {
            $this->dentia_generate_file_options();
			$this->dentia_generate_file();
		}
	}

    function dentia_generate_file_options() {
        ...
        $this->redux->filesystem->execute( 'put_contents', $_options, array(
            'content' => preg_replace( "/(?<=[^\r]|^)\n/", "\r\n", $this->dentia_options_output() )
        ) );
    }

	function dentia_generate_file() {
		$scss_dir = get_template_directory() . '/assets/scss/';
	...
	protected function dentia_options_output() {
		$theme_colors                    = dentia_configs('theme_colors');
        $links                           = dentia_configs('link');
        $gradients                       = dentia_configs('gradient');
		ob_start();

		printf('$dentia_theme_colors_key:(%s);',$this->print_scss_opt_colors($theme_colors,'key'));
        printf('$dentia_theme_colors_val:(%s);',$this->print_scss_opt_colors($theme_colors,'val'));
        // color rgb only
        ...
        if(isset($theme_default['font-family'])) {
            if($theme_default['font-family'] == false) {
                echo '
                    $ft_theme_default: "Bebas Neue", Helvetica, Arial, sans-serif;
                ';
            } else {
                echo '
    ...
    /* Inline CSS */
    function dentia_enqueue() {
        $css = $this->dentia_inline_css();
        if ( !empty( $css ) ) {
            wp_add_inline_style( 'pxl-style', $css );
        }
    }
    protected function dentia_inline_css() {
        ob_start();
