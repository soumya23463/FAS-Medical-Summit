...
if (!class_exists('dentia_Page')) {

    class dentia_Page
    {
        public function get_site_loader(){

            $site_loader = dentia()->get_theme_opt( 'site_loader', false );
            $loader_style = dentia()->get_theme_opt( 'loader_style', 'style-digital' );
            $gradient_color = dentia()->get_theme_opt( 'gradient_color' );
            if($site_loader) { ?>
        ...
        public function get_page_title(){
            ...
            $pt_mode = dentia()->get_opt('pt_mode');
            ...
            if ($pt_mode == 'bd' && $ptitle_layout > 0 && class_exists('Pxltheme_Core') && is_callable( 'Elementor\Plugin::instance' )) {
                ...
                <div id="pxl-page-title-default" class="pxl--parallax" data-stellar-background-ratio="0.5">
                    <div class="container">
                        <div class="row">
                            <div class="col-lg-12">
                                <h1 class="pxl-page-title wow fadeInUp"><?php echo dentia_html($titles['title']) ?></h1>
        ...
        public function get_title() {
            ...
            if ( ! is_archive() ) {
                ...
                elseif ( is_page() ) {
                    ...
                    $title = esc_html__( '404 Error', 'dentia' );
                } elseif ( is_search() ) {
                    $title = esc_html__( 'Search results', 'dentia' );
                } elseif ( is_singular('lp_course') ) {
                    $title = esc_html__( 'Course', 'dentia' );
                } else {
                    $title = get_post_meta( get_the_ID(), 'custom_title', true );
                    if ( ! $title ) {
                        $title = get_the_title();
        ...
        public function get_breadcrumb(){
            ...
            foreach ( $entries as $entry )
            {
                ...
                $pxl_post_title = dentia()->get_opt( 'pxl_post_title', 'default' );
                $pxl_post_title_text = dentia()->get_opt( 'pxl_post_title_text' );
        ...
        public function get_pagination( $query = null, $ajax = false ){
            ...
            if($ajax){
                add_filter('paginate_links', 'dentia_ajax_paginate_links');
            }
